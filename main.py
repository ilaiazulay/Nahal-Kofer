...
def schedule_task(interval, task_function):
...
def run_scheduler():
    ...
    scheduler_thread = threading.Thread(target=schedule_task, args=(10, get_water_level_alert))
    scheduler_thread.start()

    # Run the save daily sensor reading scheduler in a separate thread
    daily_interval = 24 * 60 * 60  # 24 hours in seconds
    save_sensor_thread = threading.Thread(target=schedule_task, args=(daily_interval, save_daily_sensor_reading))
    save_sensor_thread.start()

    # Run the check flood risk scheduler in a separate thread with a daily interval
    check_flood_thread = threading.Thread(target=schedule_task, args=(daily_interval, check_flood_risk))
...
def save_daily_sensor_reading():
    ...
    with app.app_context():  # Use the passed app instance for the application context
        from website.models import Sensor, LabTest
        from website.views import get_distance_sensor_data, get_flow_sensor_data, get_ph_sensor_data
